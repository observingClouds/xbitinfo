...
def to_compressed_netcdf(ds_bitrounded, ds, path, shuffle=True, complevel=9, for_cdo=True, time_dim="time", chunks=None):
    """Save bitrounded xr.Dataset to_netcdf."""
    ds_bitrounded.to_netcdf(path, encoding={v:{'zlib':True, 'shuffle':shuffle, 'complevel':complevel,
             'chunksizes':get_chunksizes(ds_bitrounded[v], ds[v], for_cdo=for_cdo, time_dim=time_dim, chunks=chunks)} for v in keepbits})
